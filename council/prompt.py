...
def discussion_prompt(round_num: int) -> str:
    ...
    return (
        f"You are participating in a council discussion, Round {round_num} of 3.\n\n"
        f"{round_guidance}\n\n"
        f"{engagement}\n\n"
        f"{brevity}\n\n"
        f"{response_format}"
...
def vote_prompt(motion: str) -> str:
    ...
    return (
        f"{template.replace('{motion}', motion)}\n\n"
        f"{engagement}\n\n"
        f"{vote_format}"
    )
