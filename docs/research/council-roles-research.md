...
```mermaid
flowchart LR
  Q["Decision Question"] --> C["Creator<br/>(generate alternatives)"]
  Q --> S["Scout<br/>(seek info & options)"]
  Q --> K["Skeptic<br/>(critique & dissent)"]
  Q --> I["Implementer<br/>(plan & follow-through)"]
  Q --> G["Guardian<br/>(health, values, cohesion)"]

  C -->|more options| K
  S -->|better inputs| K
  K -->|select best bet| I
  G -->|reduce burnout & conflict| I
  G -->|keeps dissent safe| K
```
...
```mermaid
flowchart TD
  A["Collect sources<br/>(primary + meta-analyses)"] --> B["Extract behaviors + tradeoffs<br/>(what they do / fail at)"]
  B --> C["Cluster into archetypes<br/>(redundant models collapse)"]
  C --> D["Map to decision functions<br/>(idea, scout, critique, execute, care)"]
  D --> E["Write persona spec<br/>(voice, script, triggers, anti-patterns)"]
  E --> F["Test on past decisions<br/>(retrospective decision log)"]
  F --> G["Adjust weights + scripts<br/>(calibrate vs your failures)"]
  G --> H["Deploy as a ritual<br/>(5-minute council, then act)"]
```
