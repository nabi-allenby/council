...
def test_discussion_prompt_loads():
    ...
    for round_num in (1, 2, 3):
        prompt = discussion_prompt(round_num)
...
def test_vote_prompt_loads():
    """vote_prompt() loads files and inserts the motion."""
    prompt = vote_prompt("Pull the lever.")
    assert "Pull the lever." in prompt
...
def test_council_completes():
    ...
    orchestrator = Orchestrator(agents=make_mock_agents())
...
def test_decision_record_format():
    """Decision record contains outcome, motion, and all 5 votes."""
    orchestrator = Orchestrator(agents=make_mock_agents())
    session = orchestrator.run(TROLLEY_QUESTION)
    ...
    assert "Creator" in record
    assert "Scout" in record
    assert "Skeptic" in record
    assert "Implementer" in record
    assert "Guardian" in record
...
def test_motion_is_implementer_round3():
    """The motion comes from the Implementer's final round position."""
    orchestrator = Orchestrator(agents=make_mock_agents())
    session = orchestrator.run(TROLLEY_QUESTION)

    impl_r3 = [t for t in session.turns if t.agent == "implementer" and t.round == 3]
    assert len(impl_r3) == 1
    assert session.motion == impl_r3[0].parsed.position


def test_transcript_builds_incrementally():
    """Turns are ordered: all 5 agents per round, 3 rounds total."""
    orchestrator = Orchestrator(agents=make_mock_agents())
    session = orchestrator.run(TROLLEY_QUESTION)

    # Round 1
    assert session.turns[0].agent == "creator" and session.turns[0].round == 1
    assert session.turns[1].agent == "scout" and session.turns[1].round == 1
    assert session.turns[4].agent == "guardian" and session.turns[4].round == 1

    # Round 2 starts at index 5
    assert session.turns[5].agent == "creator" and session.turns[5].round == 2

    # Round 3 starts at index 10
    assert session.turns[10].agent == "creator" and session.turns[10].round == 3
...
def test_trolley_e2e():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "council", TROLLEY_QUESTION],
        capture_output=True,
        text=True,
        timeout=300,
    )
...
if __name__ == "__main__":
    tests = [
        ...
        test_motion_is_implementer_round3,
